...
extension Dictionary where Key: StringProtocol, Value: Any {
    ...
    subscript(keyPath keyPath: JSONKeyPath) -> Any? {
        get { return value(keyPath: keyPath, reference: nil) }
        set { setValue(newValue as Any, keyPath: keyPath) }
    }

    subscript(keyPath keyPath: JSONKeyPath, reference reference: Any?) -> Any? {
        get { return value(keyPath: keyPath, reference: reference) }
        set { setValue(newValue as Any, keyPath: keyPath) }
    }
    
    func exists(keyPath: JSONKeyPath, reference: Any? = nil) -> Bool {
...
struct IfHandler: KeyPathHandler {
    ...
    func value(keyPath: JSONKeyPath, input: Any?, reference: Any?) -> Any? {
        ...
        let blank = conditional?[keyPath: JSONKeyPath("blank"), reference: reference]
        let exists = conditional?[keyPath: JSONKeyPath("exists"), reference: reference]
        let then = conditional?[keyPath: JSONKeyPath("then"), reference: reference]
        let elseCase = conditional?[keyPath: JSONKeyPath("else"), reference: reference]
